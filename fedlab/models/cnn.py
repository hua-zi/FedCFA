...
class AlexNetHook2_CIFAR10(nn.Module):
    def __init__(self, num_classes=10):
        super(AlexNetHook2_CIFAR10, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
...
class VGGHook2(nn.Module):
    def __init__(self, vgg):
        super(VGGHook2, self).__init__()
        self.features, self.features2 = self._make_layers(vgg)
        self.dense = nn.Sequential(
            nn.Linear(512, 4096),
            nn.ReLU(inplace=True),
            nn.Dropout(0.4),
    ...
    def decoder(self, latent):
        latent = self.features2(latent)
        out = latent.view(latent.size(0), -1)
        out = self.dense(out)
        out = self.classifier(out)
        return out        
    ...
    def _make_layers(self, vgg):
        ...
        layers += [nn.AvgPool2d(kernel_size=1, stride=1)]
        return nn.Sequential(*layers[:7]), nn.Sequential(*layers[7:])