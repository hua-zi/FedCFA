...
from .basic_dataset import FedDataset, BaseDataset, CIFARSubset
from ...utils.dataset.partition import CIFAR10Partitioner, VisionPartitioner, CIFAR100Partitioner
...
class PartitionedCIFAR10(FedDataset):
    ...
    def get_dataloader(self, cid, batch_size=None, type="train"):
        ...
        data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=self.shuffle) # shuffle=true 使得fedavg 和 ditto 不一样
        return data_loader

class LPartitionedCIFAR10(PartitionedCIFAR10):
    def __init__(self,
                 root,
                 path,
                 dataname,
    ...
    def preprocess(self,
        ...
        trainset = torchvision.datasets.CIFAR10(root=self.root,
                                                ...
                                                transform=self.transform,
                                                download=self.download)
        partitioner = VisionPartitioner(trainset.targets,
                                        self.num_clients,
                                        partition=partition,
                                        dir_alpha=dir_alpha,
                                        major_classes_num=2,
...
class PartitionCIFAR(FedDataset):
    ...
    def get_dataloader(self, cid, batch_size=None, type="train"):
        ...
        dataset = self.get_dataset(cid, type)
        batch_size = len(dataset) if batch_size is None else batch_size
        data_loader = DataLoader(dataset, batch_size=batch_size,shuffle=self.shuffle)
        return data_loader
