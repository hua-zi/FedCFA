...
class ModelMaintainer(object):
    ...
    def model_parameters(self) -> torch.Tensor:
        """Return serialized model parameters(model.state_dict())."""
        return SerializationTool.serialize_model(self._model)
    
    @property
    def model_grad_parameters(self) -> torch.Tensor:
        """Return serialized model parameters(model.parameters())."""
        parameters = [param.data.view(-1) for param in self._model.parameters()]
    ...
    def model_grads(self) -> torch.Tensor:
        """Return serialized model gradients(model.state_dict())."""
